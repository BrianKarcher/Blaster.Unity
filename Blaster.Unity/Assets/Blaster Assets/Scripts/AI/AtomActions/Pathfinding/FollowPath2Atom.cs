...
{
    public class FollowPath2Atom : AtomActionBase
    {
        ...
        public override void Start(IEntity entity)
        {
            ...
            _physicsComponent.GetSteering().TurnOn(behavior_type.follow_path);
            _physicsComponent.Controller.GetPhysicsData().TurnMode = PhysicsData.TurnModeEnum.TowardsSteeringBehaviorTarget;

            _behavior = _steering.GetSteeringBehavior(behavior_type.follow_path) as BlueOrb.Physics.SteeringBehaviors2D.FollowPath2;            
            ...
            WaypointChanged(Vector2.zero);
        }

        private void WaypointChanged(Vector2 force)
        {
            if (!_behavior.Path.CurrentWaypoint(out var wayPoint))
                return;
            _physicsComponent.GetSteering().SetTarget3(wayPoint.xz());
        ...
        public override void StartListening(IEntity entity)
        {
            ...
            newPathId = MessageDispatcher.Instance.StartListening("NewPath", entity.GetId(), (data) =>
            {
                ...
                var newPath = data.ExtraInfo as List<Vector2>;
                Path = newPath;
                _behavior.SetPath(Path);
...
//        }
        ...
        {
            base.End();
            _physicsComponent.Controller.GetPhysicsData().TurnMode = PhysicsData.TurnModeEnum.TowardsSteeringForce;
            _physicsComponent.GetSteering().TurnOff(behavior_type.follow_path);
